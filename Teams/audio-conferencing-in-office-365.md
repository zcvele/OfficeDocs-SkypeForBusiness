...
This article is for administrators and IT professionals who are evaluating Audio Conferencing for Microsoft Teams meetings.

Audio Conferencing allows users to join a Teams meeting from a phone using a Public Switched Telephone Network (PSTN) phone number. Audio Conferencing is sometimes called dial-in conferencing or PSTN conferencing.

Audio Conferencing requires an extra Teams add-on license for each person in your organization who is going to schedule/host an Audio Conferencing meeting. Meeting attendees who dial in don't need any extra licenses assigned to them. For more information, see [Audio Conferencing add-on licensing](./teams-add-on-licensing/microsoft-teams-add-on-licensing.md#audio-conferencing-licenses). (Audio Conferencing doesn't require a Teams Phone license.)  
...
- [**Shared numbers**](#shared-phone-numbers) are numbers that can be shared with users in *other* Microsoft 365 organizations. You *can't* change the languages that are used when a user calls in to a shared number.
...
For most countries and regions, shared numbers are automatically assigned as toll numbers. However, there are exceptions described in the following note.
