...
1. [Configure Test User Accounts](test-users-and-settings.md#testuser) to be used by these watcher nodes. If you're using the Negotiate authentication method, you must also use the **Set-CsTestUserCredential** cmdlet to enable these test accounts for use on the watcher node.
...
You must include not only the SIP address, but also the user name and password. If you don't include the password, the Set-CsTestUserCredential cmdlet prompts you to enter that information. The user name can be specified by using the domain name\user name format shown in the preceding code block.
...
Information similar to the following is returned for each user:
...
If you want to enable the PSTN test, which verifies connectivity with the public switched telephone network, you need to do some other configuration when setting up the watcher node. First, you must associate your test users with the PSTN test type by running a command similar to this from the Skype for Business Server Management Shell:
...
Because the **New-CsWatcherNodeConfiguration** cmdlet was called without using the Tests parameter, only the Default synthetic transactions (and the specified extended synthetic transaction) are enabled for the new watcher node. Therefore, the watcher node tests the following components:
...
An error occurs if one or more of these tests (for example, DataConference) is enabled on the watcher node. In this case, you receive an error message similar to the following:
...
When this error occurs, no changes are applied. The command should be re-run with the duplicate test removed.
...
This command returns information similar to this, depending on the synthetic transactions that are assigned to the node:
...
To verify that the watcher node is configured correctly, type the following command from the Skype for Business Server Management Shell:
...
By default, watcher nodes are designed to periodically run all their enabled synthetic transactions. At times, however, you might want to suspend those transactions. For example, if the watcher node is temporarily disconnected from the network, then there's no reason to run the synthetic transactions. Without network connectivity, those transactions fail. To temporarily disable a watcher node, run a command similar to this from the Skype for Business Server Management Shell:
  
```PowerShell
Set-CsWatcherNodeConfiguration -Identity "atl-watcher-001.litwareinc.com" -Enabled $False
```

This command disables the execution of synthetic transactions on the watcher node atl watcher 001.litwareinc.com. To resume execution of the synthetic transactions, set the Enabled property back to True ($True):
...
If your watcher node computer is located outside your perimeter network, you'll probably not be able to run the Data Conferencing Synthetic Transaction unless you first disable the Windows Internet Explorer® Internet browser proxy settings for the Network Service account by completing the following steps:
  
1. On the watcher node computer, select **Start**, select **All Programs**, click **Accessories**, right-click **Command Prompt**, and then click **Run as administrator**.
...
- A PSTN gateway that accepts call and media that routes call back to a receiver's home pool, based on the number dialed.
...
It might take some time for the test user contact lists to migrate to Exchange. To monitor the migration progress, the same command line can be run without the -Setup flag:
...
In this example, a Skype for Business Server rule needs to exist to route messages for litwareinc.com to an XMPP gateway.
...
For more information about how to run the VIS Synthetic Transaction,see the [Test-CsP2PVideoInteropServerSipTrunkAV](/powershell/module/skype/Test-CsP2PVideoInteropServerSipTrunkAV) cmdlet.
...
If it's desirable to run synthetic transactions more frequently, the number of synthetic transactions run with a given set of users should be decreased so that the tests can complete in the desired time range with some buffer for occasional network delays. If running more synthetic transactions is desirable, create more user sets to run other synthetic transactions.
  
To change the frequency at which synthetic transactions run, follow these steps:
  
1. Open System Center Operations Manager. Select Authoring section. Click Rules section (under Authoring).
